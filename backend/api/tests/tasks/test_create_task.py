...
@pytest.mark.django_db
def test_create_task_when_invalid_dates_are_given_then_exception_is_raised(start_date,end_date):
...
@pytest.mark.parametrize("cost_amount,cost_currency", [
    (12,"USD"),
    (12.23, "GBP"),
    (0,"PLN"),
    (9999999999.99,"GBP")
...
@pytest.mark.django_db
def test_create_task_when_valid_costs_variables_are_given_then_is_created(cost_amount,cost_currency):
    Task.objects.create(title="Test", cost_amount=cost_amount, cost_currency=cost_currency)
    assert Task.objects.count() == 1
...
@pytest.mark.django_db
def test_create_task_when_invalid_costs_variables_are_given_then_exception_is_raised(cost_amount,cost_currency):
...
@pytest.mark.parametrize("est_hours", [
-1,1.222,10000
])
@pytest.mark.django_db
def test_create_task_when_invalid_est_hours_is_given_then_exception_is_raised(est_hours):
