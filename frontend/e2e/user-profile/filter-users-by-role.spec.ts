...
test("GIVEN admin z tabelą WHEN filtruje po roli THEN każdy wiersz ma poprawny badge roli", async ({
  ...
  for (let i = 0; i < adminCount; i++) {
    await expect(adminRows.nth(i).locator(".ap__role-badge")).toContainText(
      "Administrator",
    );
  }
  ...
  for (let i = 0; i < viewerCount; i++) {
    await expect(viewerRows.nth(i).locator(".ap__role-badge")).toContainText(
      "Viewer",
    );
  }

  await page.selectOption("#role-filter-select", "all");
  expect(await page.locator("tbody tr.ap__row").count()).toBeGreaterThanOrEqual(
    1,
  );
});
