...
test('Add task by funding site', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Np. Przygotować ofertę' }).fill('Nowe zadanie do fundingu');
  await page.getByRole('textbox', { name: 'Opcjonalnie' }).fill('opis');
  await page.getByRole('button', { name: 'Zapisz' }).click();
  await expect(page.getByRole('heading', { name: 'Nowe zadanie do fundingu' })).toBeVisible();
});