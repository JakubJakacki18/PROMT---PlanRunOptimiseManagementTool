...
test("logowanie i tworzenie zadania przypisanego do projektu #10", async ({
  ...
  await expect(projectSelect.locator("option").nth(1)).toBeAttached({
    timeout: 10000,
  });
  ...
  expect(
    targetValue,
    'Nie znaleziono projektu "Projekt 10" — uruchom: dce backend python manage.py seed',
  ).not.toBeNull();
  ...
  await expect(
    page.getByRole("heading", { name: "ABC" }).first(),
  ).toBeVisible();
});
