...
test(
  "storageState — sesja admina daje dostęp do /dashboard/projects bez ponownego logowania",
  async ({ page }) => {
...
test(
  "storageState — sesja admina pozwala odpytać /api/projects/ przez request fixture bez przechodzenia przez UI",
  async ({ request }) => {
    const response = await request.get("/api/projects/");

    expect(
      response.status(),
      `GET /api/projects/ powinien zwrócić 200 OK dla zalogowanego admina. Otrzymano: ${response.status()}`,
    ).toBe(200);
...
test(
  "storageState — nieuwierzytelniony request do /api/projects/ zwraca 401 lub 403",
  async ({ playwright }) => {
